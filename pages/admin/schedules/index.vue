<template>
  <div class="content panel tw-flex tw-flex-col">
    ...
    <ListTable :soft-deletes="true" url="/admin/schedules" :filters="{ perPage: 10, sort: 'all', status: 'all', order: 'desc', search: '' }">
      ...
      <div slot="row" slot-scope="props" class="tw-flex-1">
        <div class="list-datatables-item">
          ...
          <div class="tw-self-center tw-flex tw-flex-row tw-space-x-2">
            ...
            <div v-else>
              <NuxtLink :to="{ name: 'admin-schedules-scheduleId-edit', params: { scheduleId: props.row.id } }" class="btn btn-sm btn-warning btn-rounded" tag="button" style="padding: 6px 9px 5px 9px;">
      ...
      <div slot="footer">
        <div class="tw-flex tw-flex-row">
          <Button text="TAMBAH" :styles="[ 'big', 'blue' ]" :icon="['fas', 'plus']" :to="{ name: 'admin-schedules-create' }" />
        </div>
      </div>
    </ListTable>
  </div>
...
export default {
  ...
  setup () {
    ...
    const back = () => redirect({ name: 'admin' })
