<template>
  <div class="content panel tw-flex tw-flex-col">
    ...
    <ListTable v-if="schema" :url="`/admin/schemas/${schema.id}/schedules`" :filters="{ perPage: 10, order: 'desc', search: '' }">
      ...
      <div slot="action-select" slot-scope="props">
        <button class="btn btn-sm btn-danger" @click="props.deleteItem(props.getChecked(e => e.schedule_id))">
      ...
      <div slot="row" slot-scope="props" class="tw-flex-1">
        <div class="list-datatables-item">
          ...
          <div class="tw-self-center tw-flex tw-flex-row tw-space-x-2">
            <button class="btn btn-sm btn-danger btn-rounded" style="padding: 6px 10px 5px 10px;" @click="props.deleteItem(props.row.schedule_id)">
      ...
      <div slot="footer">
        <div class="tw-flex tw-flex-row">
          <Button text="TAMBAH" :styles="[ 'big', 'blue' ]" :icon="['fas', 'plus']" :to="{ name: 'admin-schemas-schemaId-manage-schedules-add' }" />
        </div>
      </div>
    </ListTable>
  </div>
