<template>
  <div class="content panel tw-flex tw-flex-col">
    ...
    <ListTable v-if="schema" :url="`/admin/schemas/${schema.id}/places`" :filters="{ perPage: 10, order: 'desc', search: '' }">
      ...
      <div slot="action-select" slot-scope="props">
        <button class="btn btn-sm btn-danger" @click="props.deleteItem(props.getChecked(e => e.place_id))">
      ...
      <div slot="footer">
        <div class="tw-flex tw-flex-row">
          <Button text="TAMBAH" :styles="[ 'big', 'blue' ]" :icon="['fas', 'plus']" :to="{ name: 'admin-schemas-schemaId-manage-places-add' }" />
        </div>
      </div>
    </ListTable>
  </div>
