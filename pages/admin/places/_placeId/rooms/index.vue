<template>
  <div class="content panel tw-flex tw-flex-col">
    <div class="panel-breadcrumb">
      <Breadcrumb page="Ruangan" :data="[{text: 'Tempat Uji',link: '/admin/places'},{text: ((place) ? place.name : '') }]" />
    </div>
    ...
    <div v-if="place" class="tw-w-full tw-py-1 tw-px-4 tw-text-center tw-rounded-none md:tw-rounded-t-lg tw-bg-gray-300 tw-text-gray-700" v-text="(place) ? place.name : ''" />
    <ListTable v-if="place" :soft-deletes="true" :url="`/admin/places/${place.id}/rooms`" :filters="{ perPage: 10, order: 'desc', status: 'all', search: '' }">
      ...
      <div slot="row" slot-scope="props" class="tw-flex-1">
        <div class="list-datatables-item">
          ...
          <div class="tw-self-center tw-flex tw-flex-row tw-space-x-2">
            ...
            <div v-else>
              <NuxtLink :to="{ name: 'admin-places-placeId-rooms-roomId-edit', params: { placeId, roomId: props.row.id } }" class="btn btn-sm btn-warning btn-rounded" tag="button" style="padding: 6px 9px 5px 9px;">
      ...
      <div slot="footer">
        <div class="tw-flex tw-flex-row">
          <Button text="TAMBAH" :styles="[ 'big', 'blue' ]" :icon="['fas', 'plus']" :to="{ name: 'admin-places-placeId-rooms-create', params: { placeId } }" />
        </div>
      </div>
    </ListTable>
  </div>
...
export default {
  ...
  setup () {
    ...
    const back = () => redirect('/admin/places')
